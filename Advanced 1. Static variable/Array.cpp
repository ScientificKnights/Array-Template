...
template<typename T>
unsigned int Array<T>::DefaultSize()		//returns the default size of the array
{
	return m_size_default;
}

template<typename T>
void Array<T>::DefaultSize(unsigned int new_size)		//reset the default size of the array
{
	m_size_default = new_size;
}

template<typename T>
void Array<T>::SetElement(unsigned int index, const T & newpoint)			//sets an element
{
