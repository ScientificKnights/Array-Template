...
Point Array::GetElement(unsigned int index) const		//return the element by reference
{
	if (index < m_size)	//inside bounds
	{
		...
		return m_data[0];		//the first point
}
...
//for const Array, if Array[index] return Point &, it means Array can be changed.
//so we need return type const Point & and also function as const since we 
//don't want to change const Array object.
